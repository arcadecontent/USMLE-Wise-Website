<?php
$pageTitle = "Privacy Policy | USMLE Wise";
$pageDescription = "Privacy Policy for USMLE Wise — how we collect, use, and protect your information.";
$canonical = "https://usmlewise.com/policy";
$bodyClass = "msp";
$stylesheets = [
    "/styles/match.css",
    "/styles/legal.css"
];
$scripts = [
    "/js/match.js"
];
include $_SERVER['DOCUMENT_ROOT'] . '/partials/head.php';
?>

<main>
      <div class="msp-policy">
        <h1>Privacy Policy</h1>
        <p class="msp-policy__meta">Last updated: August 2026</p>

        <p>USMLE Wise (&ldquo;we,&rdquo; &ldquo;us,&rdquo; or &ldquo;our&rdquo;) operates the website usmle-wise.com and provides coaching, research, and residency match services to international medical graduates. This Privacy Policy explains what information we collect, how we use it, and your rights regarding your data.</p>

        <p>By using our website or services, you agree to the practices described in this policy.</p>

        <h2>1. Information We Collect</h2>
        <p>We collect information in the following ways:</p>
        <ul>
          <li><strong>Information you provide directly</strong> &mdash; When you book a guidance call, inquire via email, or contact us through WhatsApp, you may share your name, email address, phone number, country of residence, and details about your medical training and USMLE status.</li>
          <li><strong>Information collected automatically</strong> &mdash; When you visit our website, we may collect standard technical data such as your IP address, browser type, pages visited, and time spent on pages. This data is used in aggregate and is not linked to your identity.</li>
          <li><strong>Communications</strong> &mdash; If you email us or message us on WhatsApp, we retain those messages to respond to your inquiries and improve our services.</li>
        </ul>

        <h2>2. How We Use Your Information</h2>
        <ul>
          <li>To respond to your inquiries and schedule consultations</li>
          <li>To deliver the coaching, research mentorship, or match services you have enrolled in</li>
          <li>To send you program updates, scheduling information, and relevant resources related to your enrollment</li>
          <li>To improve our website and the quality of our services</li>
          <li>To comply with applicable legal obligations</li>
        </ul>
        <p>We do not sell, rent, or share your personal information with third parties for their own marketing purposes.</p>

        <h2>3. Third-Party Services</h2>
        <p>We use the following third-party platforms in the course of delivering our services. Each has its own privacy policy:</p>
        <ul>
          <li><strong>Booking &amp; scheduling</strong> &mdash; Guidance calls are booked through an external scheduling platform. Information you enter during booking (name, email, notes) is processed by that platform.</li>
          <li><strong>WhatsApp</strong> &mdash; We communicate with students via WhatsApp. Messages sent through WhatsApp are subject to Meta&rsquo;s privacy policy.</li>
          <li><strong>Google Fonts &amp; CDN services</strong> &mdash; Our website loads fonts and icon libraries from Google and unpkg. These services may collect minimal technical data as part of serving those assets.</li>
          <li><strong>Meta Pixel (advertising measurement)</strong> &mdash; Our Site includes the Meta Pixel, which tells Meta when a page is viewed and when key actions are taken (for example, starting an enrolment, booking a guidance call, or contacting us). We use this to measure how our ads perform and to show relevant ads to people who have visited us. It runs across the Site, and its use of your data is governed by <a href="https://www.facebook.com/privacy/policy/" target="_blank" rel="noopener noreferrer">Meta&rsquo;s Privacy Policy</a>.</li>
          <li><strong>Social media platforms</strong> &mdash; We maintain pages on Instagram, Facebook, LinkedIn, TikTok, and YouTube. Interactions on those platforms are governed by the respective platform&rsquo;s privacy policy.</li>
        </ul>

        <h2>4. Cookies</h2>
        <p>We use Google Analytics to understand aggregate site traffic, which sets cookies on your device. The Meta Pixel also sets an advertising cookie across the Site so we can measure our ads and reach people who have visited us; we use no other advertising or retargeting cookies. We do not set any first-party tracking cookies of our own. No personal data is stored in cookies. See our <a href="/cookie-policy">Cookie Policy</a> for full detail and how to manage cookies.</p>

        <h2>5. Data Retention</h2>
        <p>We retain your information for as long as necessary to provide our services and fulfill the purposes described in this policy. If you request deletion of your data, we will remove it within a reasonable timeframe, except where retention is required by law.</p>

        <h2>6. Data Security</h2>
        <p>We take reasonable technical and organizational measures to protect your information from unauthorized access, disclosure, or loss. However, no method of transmission over the internet is completely secure, and we cannot guarantee absolute security.</p>

        <h2>7. Children&rsquo;s Privacy</h2>
        <p>Our services are intended for medical graduates and adult students. We do not knowingly collect information from anyone under the age of 18. If you believe a minor has submitted information to us, please contact us and we will delete it promptly.</p>

        <h2>8. Your Rights</h2>
        <p>Depending on your location, you may have the right to:</p>
        <ul>
          <li>Access the personal data we hold about you</li>
          <li>Request correction of inaccurate information</li>
          <li>Request deletion of your data</li>
          <li>Withdraw consent for communications at any time</li>
        </ul>
        <p>To exercise any of these rights, contact us at <!--email_off--><a href="mailto:admin@usmlewise.com">admin@usmlewise.com</a><!--/email_off-->.</p>

        <h2>9. Changes to This Policy</h2>
        <p>We may update this Privacy Policy from time to time. When we do, we will revise the &ldquo;Last updated&rdquo; date at the top of this page. Continued use of our website after any changes constitutes acceptance of the updated policy.</p>

        <h2>10. Contact Us</h2>
        <p>If you have questions or concerns about this Privacy Policy or how your data is handled, please reach out:</p>
        <ul>
          <li>Email: <!--email_off--><a href="mailto:admin@usmlewise.com">admin@usmlewise.com</a><!--/email_off--></li>
          <li>Website: <a href="/">usmle-wise.com</a></li>
        </ul>

        <hr />
        <p style="font-size:14px; color: var(--uw-ink-500);">&copy; 2026 USMLE Wise. All rights reserved.</p>
      </div>
    </main>

    <!-- ============== FOOTER ============== -->

<?php include $_SERVER['DOCUMENT_ROOT'] . '/partials/footer.php'; ?>
