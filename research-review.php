<?php
$pageTitle = "Systematic Review &amp; Meta-Analysis — Fast-track Publication Mentorship | USMLE Wise";
$pageDescription = "A mentor-guided systematic review or meta-analysis. A rigorous, credible first-author publication for your residency application. $199 — Live, mentored.";
$canonical = "https://usmlewise.com/research-review";
$bodyClass = "msp";
$stylesheets = [
    "/styles/research-catalyst.css",
    "/styles/research-program.css"
];
$scripts = [
    "/js/research-catalyst.js"
];
include $_SERVER['DOCUMENT_ROOT'] . '/partials/head.php';
?>
<main>

      <!-- ============== HERO ============== -->
      <section class="msp-hero" aria-labelledby="heroTitle">
        <div class="msp-wrap msp-hero__grid">
          <div class="msp-hero__copy reveal">
            <h1 id="heroTitle" class="msp-h1">Add a publication<br><span class="msp-h1__accent">to your CV.</span></h1>
            <p class="msp-lede">
              A mentor-guided systematic review or meta-analysis. A rigorous,
              credible first-author publication before your residency application.
            </p>
            <div class="msp-cta-row">
              <a
                class="btn btn--primary btn--xl"
                href="https://team.manikmadaan.com/guidance-call/book"
                target="_blank"
                rel="noopener noreferrer"
                >Book a Free Call</a
              >
              <a class="btn btn--outline btn--xl" href="#includes">
                View inclusions
                <i data-lucide="arrow-down" class="ic-arrow" width="16" height="16"></i>
              </a>
            </div>
          </div>

        </div>

        <div class="msp-bar msp-wrap" aria-label="Track record">
          <div class="msp-bar__inner">
            <div class="msp-stat reveal">
              <b data-count="250" data-suffix="+">250+</b>
              <span>Students &amp; professionals guided</span>
            </div>
            <div class="msp-divider" aria-hidden="true"></div>
            <div class="msp-stat reveal">
              <b data-count="75" data-suffix="+">75+</b>
              <span>Publications &amp; presentations</span>
            </div>
            <div class="msp-divider" aria-hidden="true"></div>
            <div class="msp-stat reveal">
              <b data-count="150" data-suffix="+">150+</b>
              <span>Research projects mentored</span>
            </div>
            <div class="msp-divider" aria-hidden="true"></div>
            <div class="msp-stat reveal">
              <b data-count="16" data-suffix="+">16+</b>
              <span>Countries represented</span>
            </div>
          </div>
        </div>
      </section>

            <!-- ============== WHAT'S INCLUDED ============== -->
      <section class="msp-section rp-includes" id="includes" aria-labelledby="includesTitle">
        <div class="msp-wrap">
          <div class="msp-head reveal">
            <span class="msp-eyebrow">What you get</span>
            <h2 id="includesTitle" class="msp-h2">From topic to submission, step by step.</h2>
            <p class="msp-lede">
              Your mentor guides the entire process. You do the intellectual work;
              they make sure it leads to a submission-ready paper.
            </p>
          </div>
          <div class="rp-includes__grid">

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="lightbulb" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Topic Selection</h3>
              <p>Your mentor identifies a high-yield review topic in your target specialty that is both publishable and timely enough to matter.</p>
            </div>

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="search" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Literature Search</h3>
              <p>Structured guidance on finding, sorting, and evaluating relevant studies for your review using PubMed and other databases.</p>
            </div>

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="layout-list" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Outline &amp; Structure</h3>
              <p>A mentor-reviewed outline before you write a single word, so the framework is sound and the sections flow logically.</p>
            </div>

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="pen-line" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Manuscript Writing Support</h3>
              <p>Mentor feedback throughout the writing process, from first draft to polished submission version with every revision in between.</p>
            </div>

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="target" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Journal or Conference Selection</h3>
              <p>We identify the right publication or conference target for your work and prepare your submission to meet their specific requirements.</p>
            </div>

            <div class="rp-include reveal">
              <div class="rp-include__icon"><i data-lucide="send" width="18" height="18"></i></div>
              <h3 class="rp-include__head">Submission Guidance</h3>
              <p>Mentor support through formatting requirements, submission portals, and any follow-up communication after your paper is submitted.</p>
            </div>

          </div>
        </div>
      </section>

      <!-- ============== WHO IT'S FOR ============== -->
      <section class="msp-section rc-for-whom" aria-labelledby="forWhomTitle">
        <div class="msp-wrap">
          <div class="rc-for-whom__inner">
            <div class="rc-for-whom__copy reveal">
              <span class="msp-eyebrow">Best for</span>
              <h2 id="forWhomTitle" class="msp-h2">For applicants who need a quick, credible publication.</h2>
              <p class="msp-lede">
                Systematic reviews and meta-analyses are among the most credible and
                accessible entry points in academic medicine. With the right topic and a
                mentor in your corner, you can go from idea to submission in a matter of weeks.
              </p>
            </div>
            <ol class="rc-for-whom__list">
              <li class="reveal">
                <span class="rc-whom-num">01</span>
                <span class="rc-whom-item">You need a publication on your CV before you apply</span>
              </li>
              <li class="reveal">
                <span class="rc-whom-num">02</span>
                <span class="rc-whom-item">Your timeline is under 3 months</span>
              </li>
              <li class="reveal">
                <span class="rc-whom-num">03</span>
                <span class="rc-whom-item">You want mentor support, not a DIY template</span>
              </li>
              <li class="reveal">
                <span class="rc-whom-num">04</span>
                <span class="rc-whom-item">You're building your foundation before a larger research commitment</span>
              </li>
            </ol>
          </div>
        </div>
      </section>

      <!-- ============== PROGRAM FEE ============== -->
      <section class="msp-section" id="fee" aria-labelledby="feeTitle">
        <div class="msp-wrap">
          <div class="msp-head reveal">
            <span class="msp-eyebrow">Program fee</span>
            <h2 id="feeTitle" class="msp-h2">One fee, one mentored review.</h2>
          </div>
          <div class="rc-payment reveal">
            <div class="rc-payment__inner">
              <div>
                <span class="rc-sec-label">Investment</span>
                <div class="rc-payment__price">$199</div>
                <p class="rc-payment__note">Covers mentor guidance across topic selection, protocol and search strategy, screening and data extraction, writing, and journal submission for one systematic review or meta-analysis. Journal fees, if any, are separate. See our <a href="/refund-policy">Refund &amp; Guarantee Policy</a> for how refunds are handled.</p>
              </div>
              <a
                class="btn btn--primary"
                href="https://team.manikmadaan.com/guidance-call/book"
                target="_blank"
                rel="noopener noreferrer"
                >Book a Free Call</a
              >
            </div>
          </div>
        </div>
      </section>

      <!-- ============== FAQ ============== -->
      <section class="msp-section rc-faq" id="faq" aria-labelledby="faqTitle">
        <div class="msp-wrap">
          <div class="msp-head reveal">
            <span class="msp-eyebrow">F.A.Q</span>
            <h2 id="faqTitle" class="msp-h2">Your questions, answered.</h2>
          </div>
          <div class="accordion rc-faq__accordion reveal">
            <div class="accordion__item">
              <button class="accordion__head" type="button">
                What's the difference between a systematic review and a meta-analysis?
              </button>
              <div class="accordion__body">
                A systematic review uses a rigorous, pre-defined methodology to identify and critically appraise all relevant studies on a question. A meta-analysis goes a step further by statistically pooling data from those studies to produce a combined result. Your mentor will recommend which is more feasible based on the available literature in your target specialty.
              </div>
            </div>
            <div class="accordion__item">
              <button class="accordion__head" type="button">
                How quickly can I have something submitted?
              </button>
              <div class="accordion__body">
                Systematic reviews typically take 6&ndash;10 weeks from topic selection to submission-ready manuscript, depending on the scope and your availability. Your mentor will set a realistic schedule at kickoff and adjust if your availability changes.
              </div>
            </div>
            <div class="accordion__item">
              <button class="accordion__head" type="button">
                Can I do this alongside Step prep?
              </button>
              <div class="accordion__body">
                Yes. The workload is designed to be manageable alongside active exam preparation. Most of the effort happens in focused writing sessions rather than sustained daily work. Your mentor will pace the milestones around your schedule.
              </div>
            </div>
            <div class="accordion__item">
              <button class="accordion__head" type="button">
                Will I have first-author credit?
              </button>
              <div class="accordion__body">
                Every project is structured so you meet the ICMJE authorship criteria. You do the intellectual and writing work &mdash; your mentor guides the process. First-author credit reflects the contribution you've made. We don't write papers on your behalf.
              </div>
            </div>
          </div>
        </div>
      </section>

      <!-- ============== FINAL CTA ============== -->
      <section class="rc-cta-wrap">
        <div class="msp-wrap">
          <div class="rc-cta reveal">
            <span class="msp-eyebrow msp-eyebrow--light">Get your first publication</span>
            <h2 class="rc-cta__head">From topic to submission<br>in weeks.</h2>
            <p class="rc-cta__sub">
              Book a free 30-minute call. We'll confirm a topic, set a realistic
              timeline, and show you exactly what the process looks like.
            </p>
            <div class="rc-cta__actions">
              <a
                class="btn btn--primary btn--lg"
                href="https://team.manikmadaan.com/guidance-call/book"
                target="_blank"
                rel="noopener noreferrer"
                >Book a Free Guidance Call</a
              >
              <a class="btn--outline-light btn--lg" href="/research">Back to Research</a>
            </div>
            <p class="rc-cta__trust">Free 30-minute call. No commitment.</p>
          </div>
        </div>
      </section>

    </main>

    <!-- ============== FOOTER ============== -->

<?php include $_SERVER['DOCUMENT_ROOT'] . '/partials/footer.php'; ?>
